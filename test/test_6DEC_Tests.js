...
contract('test_6DEC_Tests', async (accounts) => {
	...
	it("[recollateralizeCERES][ACCOUNT5]: Recollateralizes the system using recollateralizeCERES()",async() => {
		...
		await col_instance_USDC.transfer(account5,usdc_amount,{from: OWNER});
	...
	it("[redeemAlgorithmicCERES][ACCOUNT5]: redeemAlgorithmicCERES (collateral ratio = 0) FROM ACCOUNT5", async() => {
		console.log(chalk.blue("============USDC redeemAlgorithmicCERES(ACCOUNT5)============"));
		const ceres_amount = new BigNumber("1000e18");
		await ceresInstance.transfer(account5,ceres_amount,{from: OWNER});
		...
		const global_collateral_ratio_after = new BigNumber(await ceresInstance.global_collateral_ratio.call()).div(BIG6).toNumber();
		console.log(chalk.blue(`global_collateral_ratio_after: ${global_collateral_ratio_after}`));

		// redeemAlgorithmicCERES
		await ceresInstance.approve(pool_instance_USDC.address, ceres_amount, { from: account5 });
		await pool_instance_USDC.redeemAlgorithmicCERES(ceres_amount, new BigNumber("1e18"), { from: account5 });
