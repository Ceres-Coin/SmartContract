...
contract('TEST SCRIPTS FOR test/test_Staking_CERES_WETH_P2.js', async (accounts) => {
	...
	it ("TEST SCRIPTS FOR stakingInstance_CERES_WETH.boostedBalanceOf(account0 - account7), check its default value for account0 - account7 is 0", async() => {		
		...
		expect(parseFloat(await stakingInstance_CERES_WETH.boostedBalanceOf.call(account3))).to.equal(0);
		expect(parseFloat(await stakingInstance_CERES_WETH.boostedBalanceOf.call(account4))).to.equal(0);
