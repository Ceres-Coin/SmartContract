...
contract('TEST SCRIPTS FOR test/test_Staking_CERES_WETH_P2.js', async (accounts) => {
	...
	it ("TEST SCRIPTS FOR stakingInstance_CERES_WETH.rewardPerToken(), check its default value is 0", async() => {
		// console.log(chalk.yellow(`rewardPerToken: ${parseFloat(await stakingInstance_CERES_WETH.rewardPerToken())}`));
		const DEFAULT_VALUE = 0;
	...
	it ("TEST SCRIPTS FOR stakingInstance_CERES_WETH.earned(account0 - account7), check its default value for account0 - account7", async() => {
		...
		expect(parseFloat(await stakingInstance_CERES_WETH.earned.call(account0))).to.equal(0);
		expect(parseFloat(await stakingInstance_CERES_WETH.earned.call(account1))).to.equal(0);
		expect(parseFloat(await stakingInstance_CERES_WETH.earned.call(account2))).to.equal(0);
		expect(parseFloat(await stakingInstance_CERES_WETH.earned.call(account3))).to.equal(0);
		expect(parseFloat(await stakingInstance_CERES_WETH.earned.call(account4))).to.equal(0);
		expect(parseFloat(await stakingInstance_CERES_WETH.earned.call(account5))).to.equal(0);
